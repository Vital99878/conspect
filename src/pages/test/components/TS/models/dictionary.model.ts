...
function mapDict<T, U>(input: Dict<T>, cb: (arg: T, key: string) => U): Dict<U> {
  const toReturn: Dict<U> = {}

  for (let key in input) {
    const value = input[key]
    toReturn[key] = cb(value, key)
  }
...
function filterDict<T>(input: Dict<T>, cb: (arg: T) => boolean): Dict<T> {
...
function reduceDict<T, V>(input: Dict<T>, reducer: (acc: V, currentValue: T) => V, initialValue: V): V {
  let toReturn = initialValue

  for (let key in input) {
    const value = input[key]
    toReturn = reducer(toReturn, value)
  }
