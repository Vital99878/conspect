...
export default class DatePicker extends HTMLElement {
  ...
  constructor() {
    ...
    this.position = DatePicker.position.includes(this.getAttribute('position'))
      ...
      : this.position
    this.visible = this.getAttribute('visible') === '' || this.getAttribute('visible') === 'true' || this.visible
  ...
  getWeekDaysElementStrings() {
    return this.calendar.weekDays.map((weekDay) => `<span>${weekDay.substring(0, 3)}</span>`).join('')
  }
